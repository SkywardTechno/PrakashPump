...
public class BitmapHandler {
    ...
    public Bitmap decodeFile(String strLogoUri) {
        ...
        try {
            ...
            Bitmap bitmapUnformated = BitmapFactory.decodeStream(mContext.getContentResolver().openInputStream(Uri.parse(strLogoUri)));

            //Convert Bitmap to base 64
            ByteArrayOutputStream stream = new ByteArrayOutputStream();
            bitmapUnformated.compress(Bitmap.CompressFormat.PNG, 70, stream);
            byte[] image = stream.toByteArray();
            base64Data = Base64.encodeToString(image, 0);


            ExifInterface exif = new ExifInterface(strLogoUri);
            orientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION, 1);
            Log.e("orientation", "" + orientation);

            return bitmapUnformated;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public String getImagePath(Bitmap bitmapToSave) {
        String mainPath = Environment.getExternalStorageDirectory().toString();
        File folderPath = new File(mainPath + "/" + mContext.getString(R.string.app_name));
        ...
        try {
            ...
            bitmapToSave.compress(Bitmap.CompressFormat.PNG, 70, fos);
            fos.flush();
            fos.close();
        ...
        Log.i("DecodeImageFrom", "File Path : " + file.getAbsolutePath());
